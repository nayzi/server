...
/**
 * Regroupe les webservices relatifs aux listes de pieces pour bon de commande
 * @class OrderPiece
 * @static
 ...
 * @namespace API
 */
class OrderPiece implements ItemCreatable, ItemUpdatable, ItemDeletable {
    ...
    public static function getItem($itemId) {
        ...
        $result = \Logics\OrderPiece::getItem($itemId);

        if ($result->isValid()) {
            $data = new Wrapper('orderPiece', $result->getData());
        } else {
    ...
    public static function getItems() {
        ...
        if (array_key_exists('ids', $args) && count($args['ids']) != 0) {
            $result = \Logics\OrderPiece::getItem($args['ids']);
        } else {
            $result = \Logics\OrderPiece::getItem();
        }
    ...
    public static function create() {
        ...
        $result = \Logics\OrderPiece::create($orderPiece['order'], $orderPiece['piece'], $orderPiece['isComputed']);
